<template>
    <div class="container headtop">
      ...
      <div class="right-wrap">
        <!-- <Tab :tabData="tabData" @tabClick="tabClick">
      </Tab> -->
        <strategy></strategy>
      </div>
    </div>
...
import strategy from "@/components/strategyInfo/index.vue";
...
export default {
  ...
  methods: {
        ...
        for (let i = 0; i < That.tableData.length; i++) {
          if (name == That.tableData[i].currencyName) {
            ...
            That.$set(
              That.tableData[i],
              "Increase",
              ((1 - That.websodata.last / That.websodata.open_24h) * 100).toFixed(2) + "%"
            );
            break;
          }
        }
    ...
    createSockfn(value) {
      let url =
        "ws://" + ip + "/webSocket/cuy-" + value + "-" + localStorage.getItem("user");
      createSocket(url);
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            if (name == this.tableData[i].currencyName) {
              ...
              this.$set(
                this.tableData[i],
                "Increase",
                ((1 - this.websodata.last / this.websodata.open_24h) * 100).toFixed(2)
              );
              break;
            }
          }
